...
public class CategoryService {
    ...
    public void uploadFiles(MultipartFile image, MultipartFile thumbnail, Integer cateId) {
        Category category = categoryRepository.findById(cateId).orElseThrow(() -> new UserException("Category not found"));
        if (image != null) {
            try {
    ...
    public Category get(Integer cateId) {
        return categoryRepository.findById(cateId).orElseThrow(() -> new UserException("Category not found"));
    }
